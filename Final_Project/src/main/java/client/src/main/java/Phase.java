...
public class Phase {
    ...
    public static final String SEASON_ID = "2019";
...
//            Runnable runnable = createRunnable(i, "1");
//            Thread thread = new Thread(runnable);
//            thread.start();
//        }
//        for (int i = 0; i < numThreads; i++) {
//            Runnable runnable = createRunnable(i, "2");
//            Thread thread = new Thread(runnable);
//            thread.start();
//        }
        for (int i = 0; i < numThreads; i++) {
            Runnable runnable = createRunnable(i, "3");
            Thread thread = new Thread(runnable);
            thread.start();
        }
    }
    ...
    public Runnable createRunnable(final int threadIndex, String dayID) {
        breaker = new EventCountCircuitBreaker(2500, 1, TimeUnit.SECONDS, 2500, 3, TimeUnit.SECONDS);
                    ...
                    if (breaker.incrementAndCheckState()) {
                        for (int j = 0; j < MAX_TRY_COUNT; j++) {
                            try {
                                ...
                                // Run client three times: vary the day id to 1, 2, 3
                                ApiResponse<Void> res = skiersApi.writeNewLiftRideWithHttpInfo(liftRide, RESORT_ID, SEASON_ID, dayID, skierId);
                                ...
                                System.out.println(e + String.format(" ERROR: %d time POST Failure with API Exception", j + 1));
                            }
                        }
